...
evalNetwork net input = foldl evalColumn input (ntWeights net)
  where
    evalColumn col mat = map stdNeuron $
                         toList $
                         multStd (fromLists [col]) mat
...
-- | Generate two distinct random coordinates for inital position of
-- players the coordinates are those of an empty square
randomCoords :: (RandomGen g) =>
     Map
  -> Rand g ((Int, Int, Int), (Int, Int, Int))
randomCoords m = do
  let ((_,_),(w,h)) = bounds m
  b2 <- getRandomR (0, bulNum) 
  b1 <- getRandomR (0, bulNum)

  let emptySquares = filter
                     (\(i,j) -> m ! (i,j) /= 'x')
                     (indices m)
...
-- @r@ is a flag telling if the initial position should be random or
-- standard returns the score of both players as a tuple
...
playNTurns :: (RandomGen g) =>
     Bool
  -> Map
  -> Network
  -> Network
  -> Int
  -> Rand g [(Maybe (Either GameState Outcome), Int)]
playNTurns random m p1 p2 n = do 
  let ((_,_),(w,h)) = bounds m
  ((x1, y1, b1),
   (x2, y2, b2)) <- if random
                    then randomCoords m
                    else return ((1, 1, bulNum),
                                 (w, h, bulNum))  
  return $
    gameGoesOn
    (take n $
...
-- against each other @r@ is a flag telling if the initial position
-- should be random or standard returns the score of each network
playTournament :: (RandomGen g) => Bool -> Map -> [Network] -> Rand g [Int]  
playTournament random m s = do
  table <- sequence $
    matrix l l
    (\(i, j) ->
       playGame random m (s !! (i-1)) (s !! (j-1)))
  return $ map
    (\x ->
        (sum $ fmap fst (getRow x table)) +
        (sum $ fmap snd (getCol x table)))
    [1..l]
  where 
    l = length s

takeNBest :: (RandomGen g) =>
     Bool
  -> Map
  -> [Network]
  -> Int
...
takeNBest random m s n = do
  ...
  return $
    take n $
    map fst (sortOn ((*(-1)).snd) $ zip s results)

makeRandomMatrix :: (RandomGen g) =>
     (Int,Int)
  -> Float
  -> Rand g (Matrix Float)
...
makeRandomMatrices :: (RandomGen g) =>
     [Int]
  -> Float
  -> Rand g [Matrix Float]
...
evolveNet random m nets = do
  ...
  third <- mapM (\x -> makeRandomNetwork netSize) $
           [1..(netNum - 2*bestNum)]
  return $ first ++ second ++ third

trainNet :: (RandomGen g) =>
     Bool
  -> Map
  -> Rand g Network
...
writeNetToFile network filename = 
  writeFile filename (netToString network)
...
readNetFromFile sizes filename = do
 ...
 return $
   Network {ntWeights =
               zipWith fromListMy
               (zip sizes $ tail sizes)
               nums}
 where 
  stringsToFloats s = map (\x -> Prelude.read x :: Float ) (words s) 
